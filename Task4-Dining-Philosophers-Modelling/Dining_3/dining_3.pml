...
init {
	atomic {	// Philosophers must wait until everybody is there before eating, these are polite philosophers
		...
		// Start with all forks unavailiable except for the ones at the side of the philosophers
		for (i : 0 .. NUMBER_OF_PHILOSOPHERS - 1) {
			fork[i] = 99
		}

        fork[NUMBER_OF_PHILOSOPHERS - 1] = -1;
        fork[1] = -1;
        fork[2] = -1;
        // Two philosophers at table, beside each other	
        run Philosopher(0);
        run Philosopher(1);
	}
